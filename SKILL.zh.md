...
**密钥安全硬约束（不可违反——codemap 受 git 跟踪，泄露的密钥会进入历史）**：

上面的 verbatim 摘录要求若不加防护，会把源码/配置里的任何 API key / token / 密码 / 私钥原样抄进 `.codemaps/<slug>.md`。两道防线，在选点与摘录两个时机施加（第三道机械兜底在 §5.0 运行）：

- **规避（选点时）**——密钥承载文件（`.env` / `.env.*`、`*.pem` / `*.key` / `*.p12`、`secrets.*`、`credentials*`、`*.tfvars`，以及任何匹配 gitignore 密钥模式的文件）一律表示为 **Pointer Node**（`**<label>** — \`file:line\``，无 fenced code）。指出配置「在哪里被加载」，绝不抄出密钥值本身。
- **脱敏（摘录时）**——若选定摘录仍含密钥字面量，把**字符串字面量内部的密钥值**替换为带类型占位符 `‹REDACTED:<kind>›`（如 `const KEY = "‹REDACTED:api-key›";`）。这是「禁止改写代码」的**唯一例外**——属于与 `// ...` 省略同类的诚实遮罩：行的形状保持真实，仅隐藏密钥 token。**不要**删除该行、用注释替换、或转述周围代码。
- **应视为密钥的模式**：带厂商前缀的 token（`AKIA…` / `ASIA…` / `AROA…` / `ghp_…` / `github_pat_…` / `xox[baprs]-…` / `sk-…` / `sk_live_…` / `AIza…`）、`-----BEGIN … PRIVATE KEY-----` 块、JWT（`eyJ….eyJ….…`）、内联凭证的连接串（`scheme://user:pass@host`），以及对 `password` / `secret` / `api_key` / `access_token` / `client_secret` / `private_key` 的带引号硬编码赋值。
...
- **退出码 1** → **schema 不合法 或 内嵌了密钥**（validate 对每个 schema 干净的文档都会跑密钥扫描）。密钥诊断形如 `<stdin>:<line>:<col> - error: possible <kind> credential embedded in codemap content (…)`，且 stderr **可能包含多个块**——每个 finding 一个。**一次性修复全部**：对每个被标记的行套用 §2 的脱敏规则（在字符串字面量内填 `‹REDACTED:<kind>›`），然后重新校验。它与下面的 3 次修复预算共享，所以不要一个一个改。schema 违规则相反，是单条诊断，格式如下：
  ```
...
- [ ] 每段 fenced code 与源文件对应行**逐字一致**（除 `// ...` 省略与 `‹REDACTED:<kind>›` 密钥遮罩外）

**密钥安全**：
- [ ] 任何 fenced code 或 frontmatter 内无明文 API key / token / 密码 / 私钥；密钥承载文件为 Pointer Node；不可避免的字面量已在字符串字面量内遮罩为 `‹REDACTED:<kind>›`
- [ ] §5.0 validate 退出码 0（它对检测到的密钥也会以退出码 1 失败）
...
2. **禁止改写代码**：fenced code block 必须是源文件真实连续行（允许 `// ...` 省略），禁止拼接 / 改写 / 生成式重构——**唯一例外**是把字符串字面量内部的密钥值遮罩为 `‹REDACTED:<kind>›`（见 §2 密钥安全硬约束）
...
17. **禁止输出明文密钥**：密钥承载文件作为 Pointer Node（无 code 摘录）；不可避免的密钥字面量在字符串字面量内遮罩为 `‹REDACTED:<kind>›`（唯一允许的代码改写）。`validate` 对任何内嵌密钥机械拒绝（退出码 1）——此处泄露会进入 git 历史（见 §2 密钥安全硬约束）
